...
**Pieter Cailliau:** Yeah, so the difference is then you have graph native, you have an O(1) time complexity to go from one node to another node. So instead of an \[unintelligible 00:16:50.14\]relation database. And typically, you try to keep the data as close as possible, residing in the same key, effectively.
